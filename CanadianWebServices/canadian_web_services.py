...
# Library of functions to help save layers in registry  for QGIS 3 change _winreg to winreg
from _winreg import *
...
class CanadianWebServices:
    ...
    def loadServiceList(self):
        ...
        if self.works == True:
            ...
            for service in services:
                ...
                if service['title'] != "" and numLayers <= 20 and perAvailable >= 90 and serviceType != 'WMTS':
    ...
    def setTableWidgetBehaviour(self):
        ...
        self.dlg.tableWidget.setColumnWidth(0, 110) # Name/Title
        self.dlg.tableWidget.setColumnWidth(1, 110) # Type
        self.dlg.tableWidget.setColumnWidth(2, 207) # Host 
        self.dlg.tableWidget.setColumnWidth(3, 86)  # number of layers
    ...
    def loadWebService(self):
        ...
        if servType == "WMS":
			...
			for layer in range(numLayers):
				...
				print (rlayer.extent().toString()) # TO DELETE
...
sys.setdefaultencoding("utf-8")
'''
	Helper function for method saveLayers that returns the title I should be using for the given key (Created to consider possibilities of services with the same name but different urls)
	
	@param title - The title of the service
	@param url - the url of the service
	@param type - the type of service (ie WMS,WFS,ESRI MapServer)
	@param counter - The amount of times the helper method has been run for a given service instance
	
	@returns title - Which is the title of the service we should be using 

'''
def saveLayers_helper(title,url,type,counter):
	counter+=1 
	try: # Checks if key with same title exists
		if(type == "WMS"): 
			val = "connections-wms"
		elif (type == "WFS"):
			val = "connections-wfs" 
		...
		key = OpenKey(HKEY_CURRENT_USER,"Software\\QGIS\\QGIS2\\Qgis\\"+val+"\\"+title,0,KEY_ALL_ACCESS) 
		
		service_added = QueryValueEx(key,"url")[0] == url # checks if the key has the same url (is the same service)
		
		if (service_added): # If it does than do nothing and return the title
			return title
		else: 
			
			if(counter == 1): # Adds the appropriate suffix to the title to differentiate the service from others of the same name(title)
				title+="_"+str(counter)
			else:
				title = title[:-len(str(counter-1))]
				title += counter
			
			return saveLayers_helper(title,url,type,counter)
	except: # If the key doesn't already exist returns title
		return title
...
'''
def saveLayers(title,url,type):
	if (sys.platform == "win32" or sys.platform == "win64"): # Checks if plugin is running on a windows system (IMPORTANT since registry works different for MAC users and LINUX users) 
		title = saveLayers_helper(title,url,type,0)
		#Following block creates and sets authentication settings for chosen service 
		##########################################################################################################################
		val = "" # will hold path to key for service
		try: # try block checks to see if the required keys have already been made
